...
class Game {
  constructor() {
    ...
    this.playerImg.forEach((choices) =>
      choices.addEventListener("click", this.start.bind(this))
    );
  ...
  start(e) {
    ...
    if (comChoice === pChoice) {
      ...
      [...this.playerImg]
        .find((pick) => pick.alt === pChoice)
        .classList.add("pick");
      [...this.playerImg]
        .filter((pick) => pick.alt !== pChoice)
        .forEach(removePick);
      [...this.comImg]
        .find((pick) => pick.alt === comChoice)
        .classList.add("pick");
      [...this.comImg]
        .filter((pick) => pick.alt !== comChoice)
        .forEach(removePick);
    }
